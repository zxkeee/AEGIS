...
>
> **Позиционирование — не здесь, а в `docs/PRODUCT.md`.** Этот файл отвечает
> только на вопрос «что готово и что делать дальше». Приоритет пункта = стадия
> воронки See → Protect → Prove, на которой он застревает (§3 PRODUCT.md).
> Ревизия 2026-09-15: файл отставал на три сессии и помечал сделанное как
> несделанное — см. B1.
...
- ✅ **Зеркальный режим** (`mirror_sink`): терминальный приёмник копии трафика,
  дренирует тело, отвечает 204, никогда не проксирует. Наблюдение помечается
  `RequestOnly`, каталог считает `responsesSeen` отдельно. `docs/mirror-mode.md`
- ✅ **Печати журнала** (`internal/forensic/seal*.go`): почасовой корень Меркла
  по записям периода, ссылка на предыдущую печать, сквозной `seq` и подписанная
  голова цепочки — удаление хвоста обнаружимо, а не только правка внутри
  периода. `docs/forensic-seals.md`
...
| P1-3 | **Интеграции/алертинг** 🟡 v1 | Сделано (см. D2): блок `alerting` в конфиге — `webhook_url` (только https, env-override `AEGIS_ALERT_WEBHOOK_URL`), формат `generic`/`slack`, порог `min_severity` (info/warning/critical), всё под `config.Validate`. Исходящий вызов идёт через `internal/safefetch`, поэтому вебхук не увести редиректом во внутреннюю сеть. **Прошлая версия строки — «сейчас webhook захардкожен пустым» — устарела.** Осталось: Splunk/Elastic-экспорт, PagerDuty/Teams-шаблоны, per-rule routing, тикетинг. |
| P1-4 | **OpenAPI / spec drift** 🟡 v1 | Сделано (см. B2): импорт OpenAPI 3 / Swagger 2, drift «задокументировано vs реально» (`internal/discovery/drift.go`, `GET /api/discovery/drift`), и **positive-security enforcement** (`middleware.SchemaValidation`, `internal/middleware/schema.go`) — query-параметры и JSON-тело против документированной операции, `additionalProperties:false` ⇒ отказ недокументированным полям (anti mass-assignment, API6), monitor/block (422). **Прошлая версия строки числила всё это несделанным.** Осталось: per-tenant загруженные спеки, path/header-параметры, форматы и bounds, автогенерация спеки из трафика. |
...
| # | Разрыв | Что сделать |
|---|--------|-------------|
| P2-1 | **Compliance-отчётность** 🟡 v1 | Сделано: `GET /api/compliance` + вкладка Compliance — находки (PII-exposure, shadow-API) и рантайм access-control abuse (BOLA/BFLA) маппятся на контроли **NIS2** (Art. 21(2)) и **ISO/IEC 27001:2022** (Annex A) через OWASP API Top-10 (напр. IDOR → NIS2 Art.21(2)(i) / ISO A.8.3), с severity и списком issues; чистая функция маппинга под тестом. **Подписанные отчёты** (`internal/attest`, `cmd/reportverify`): `GET /api/report?sign=1` и `GET /api/compliance?sign=1` возвращают конверт `{document, attestation}` — документ передаётся строкой своего JSON, подпись Ed25519 на **отдельном** ключе (`AEGIS_REPORT_SIGNING_KEY`, переиспользование другого секрета отклоняется в `config.Validate`); `GET /api/report/signing-key` публикует `key_id` для закрепления вне документа; `cmd/reportverify` — автономный верификатор, который отказывается работать без закреплённого ключа, потому что проверка отчёта против ключа из самого отчёта не доказывает ничего. `format=csv&sign=1` и отсутствие ключа → 400, а не молча неподписанный документ. **DORA-маппинг** (Regulation (EU) 2022/2554): находки и рантайм-абьюз ложатся на Art. 8(1) (инвентаризация ИКТ-активов ← пассивный каталог и shadow-эндпоинты), Art. 9(3) (конфиденциальность данных ← PII-экспозиция), Art. 9(4)(c) (логический доступ ← BOLA/BFLA), Art. 9(4)(d) (строгая аутентификация) и Art. 10(1) (детект аномалий) — последний **только по наблюдённым событиям**: статическая находка говорит, что эндпоинт *можно* злоупотребить, а не что детект сработал, и засчитывать её было бы ложью (признак `runtimeOnly` в `controlRef`). Отчёт сам называет то, чего не покрывает: DORA Art. 17–19 и NIS2 Art. 23 — жизненный цикл инцидента (группировка событий в инцидент, классификация по критериям регулятора, сроки уведомления); сущности инцидента в AEGIS нет, и список `not_evidenced` виден и в API, и в консоли, и в демо-отчёте. См. `docs/compliance-evidence.md`. **Сущность инцидента** (`internal/incident`): события коррелируются в инциденты по `(tenant, class, subject)` в окне 6 часов — по проверенной личности вызывающего, а не по адресу (смена адреса иначе дробит одну кампанию на сотни инцидентов); инцидент открывают только классы, которые регулятор признает атакой (BOLA/BFLA/WAF/behavior/threat), рутинные срабатывания контролей — нет. Жизненный цикл open→contained→closed, сроки NIS2 ст. 23(4) (24ч / 72ч / месяц **от подачи уведомления**, не от обнаружения), классификация по DORA ст. 18 с явным разделением «что видит шлюз» и «что заполняет оператор», append-only история поданных уведомлений, `GET/PATCH /api/incidents`, RLS и tenant-изоляция. Закрытие инцидента **не** снимает пропущенный срок. Статьи DORA 17/18/19 и NIS2 23 теперь евиденсятся самим реестром инцидентов и каждая по своему порогу: вести инциденты — не то же самое, что классифицировать, а классифицировать — не то же самое, что подать отчёт; непокрытое по-прежнему честно перечислено. **Целостность журнала** (`internal/forensic/seal*.go`): почасовые корни Меркла, цепочка печатей, сквозной `seq` и подписанная голова — удаление записи внутри периода И обрезка хвоста цепочки обнаружимы. **Известные границы, которые нельзя переобещать:** печати делают удаление обнаружимым, а не невозможным; ключ подписи держит сам проверяемый, поэтому подпись доказывает «документ не изменён после создания», но не «собран не из подчищенных данных»; внешнего якоря (RFC 3161 / прозрачный журнал) нет. Правильное слово — tamper-**evident**. **Открыто:** у `VerifySeals` нет продуктовой точки входа — ни admin-эндпоинта, ни команды в `reportverify`, то есть проверить печати снаружи нечем; `incidents` и `admin_audit_log` прямо или косвенно питают подписанный отчёт и защиты целостности не имеют. Дальше: точка входа для проверки печатей, страница инцидентов в консоли, PCI-DSS/HIPAA/GDPR-шаблоны, экспорт отчёта (PDF/CSV), data residency, DSAR. |
...
- ~~Приоритет рынка: inline-защита vs out-of-band observability?~~ — **снят
  2026-09-15, но не ответом, а тем, что вопрос был ложной дихотомией.**
  Out-of-band (зеркало) — это *вход*, inline — это *вторая встреча*
  (PRODUCT.md §3). Строить надо оба, в этом порядке. Что действительно остаётся
  открытым и **не решается кодом**: платит ли кто-нибудь за любой из них.
  Позиционирование к 15.09 сменилось трижды без единого разговора с покупателем.
...
|---|---|---|
| B1. **Out-of-band сенсор** ✅ mirror / ❌ eBPF | **Зеркало сделано** (`mirror_sink`, `docs/mirror-mode.md`): клиентский nginx шлёт копию запроса, ответ отбрасывается, трафик клиента не зависит от нашего процесса — его можно остановить посреди пилота и показать, что ничего не изменилось. Это вход воронки (PRODUCT.md §3) и первое, что говорит оффер. Честная граница заложена в код: копия не несёт ответа ⇒ нет PII-находок, нет BOLA, нет статусов и латентности; ноль в `pii_count` читается как «ответы не смотрели». **eBPF-сенсор не сделан и не нужен, пока зеркало не встретило живого возражения** — зеркало закрывает случай «не пущу в контур» за два дня вместо двух месяцев. | зеркало сделано; eBPF 4–8 нед, отложен |
...
**Ревизия 2026-09-15.** Прошлая версия этого списка не содержала
compliance-отчётности вообще, хотя именно туда ушли последние сессии работы.
Список перестроен по воронке `See → Protect → Prove` (PRODUCT.md §3): пункт
приоритетен настолько, насколько он расшивает стадию, на которой застревает
сделка. Пункты P2-1 и P2-2 — не «дифференциация на потом», а стадия Prove, и у
неё свой порядок внутри.

Сначала закрыть P0-блокеры релиза (аутентификация консоли, тесты), затем:

**Стадия See — вход, самая дешёвая работа с самым коротким плечом**
1. **B1 зеркало** ✅ — сделано; барьер внедрения снят.
2. **B2 OpenAPI drift** 🟡 — «задокументировано vs реально» это буквально то,
   что покупает первый пилот.
3. **C1 классификация данных** 🟡 — «эти три API светят карты без auth»
   говорит на языке покупателя, а не инженера.

**Стадия Protect — ров, за это платят**
4. **A1 BOLA/BFLA** ✅ v2 — самый ценный детект.
5. **A6 + A2** 🟡 — baseline и контроль false-positive; без них детект
   бесполезен, а FP-замер на Codeberg (101 ложная критичная находка на 161
   запрос) показал, что это не теория.
6. **A3 Account Takeover** — отдельный высокоценный сценарий, не начат.

**Стадия Prove — бюджет и продление**
7. **Точка входа для проверки печатей** — полдня. Без неё вся работа по
   целостности недостижима снаружи, то есть не существует для покупателя.
8. **P2-1 остаток** — страница инцидентов, экспорт PDF/CSV.
9. **Внешний якорь** — только если спросят. Если за десять разговоров никто не
   спросит про целостность журнала, значит рынок покупает не то, что мы думаем,
   и это тоже ответ.

**Поперёк всех стадий**
10. **D1–D3 интеграции** — SIEM/алертинг; сейчас webhook фактически пустой.
11. **B5 schema-импорт, A4/A5 → E → F** — добивание до enterprise/scale.
