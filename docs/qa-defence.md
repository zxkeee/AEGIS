...
найдено **одиннадцать тестов, которые проходили на сломанном коде**. Каждый
перечислен поимённо в `handoff.md` §0c — поэтому число одиннадцать, а не
круглее. Это редкая практика, и она проверяема.

(Здесь стояло «четырнадцать». Списка на четырнадцать не существует нигде.
Цифра, которую нельзя перечислить, — ровно то, на чём ловит технический
оценщик, и хуже всего ей стоять именно в этом документе.)
