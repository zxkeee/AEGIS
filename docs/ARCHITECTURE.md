...
- Инсайдер/оператор на admin plane (:8081) — RBAC решает, что администратору
  **можно**; журнал действий (`admin_audit_log`) записывает, что он **сделал**.
  Вторая половина фразы до 23.09 не была ничем обеспечена: у таблицы не было ни
  RLS, ни контроля целостности, а retention удалял из неё по расписанию.
  Теперь: RLS с тенантной политикой, дайджест на каждую строку и цепочка между
  строками, голова с обязательством о количестве (`admin_audit_head`),
  подписываемая тем же ключом, что и печати. Вычистка по retention
  **записывается** в голову, поэтому законное удаление не выглядит подделкой, а
  незаписанное — выглядит. `internal/audit/integrity.go`.

  Граница, которую нельзя переобещать: это tamper-**evident**. Оператор с ключом
  подписи может переписать журнал, пересчитать цепочку и переподписать голову.
  Закрывает это только внешний якорь, и его нет.
